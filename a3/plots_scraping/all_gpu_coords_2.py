...
file_name = "../results/run_kmeans_4a.out"
...
methods = ["Naive", "Transpose", "Shared", "offload"]
...
plt.plot(t_total["Naive"].keys(), [t_seq_total / t_total["Naive"][key] for key in t_total["Naive"].keys()], label="Naive", marker='o')
plt.plot(t_total["Transpose"].keys(), [t_seq_total / t_total["Transpose"][key] for key in t_total["Transpose"].keys()], label="Transpose", marker='o', color='#2CA02C')
plt.plot(t_total["Shared"].keys(), [t_seq_total / t_total["Shared"][key] for key in t_total["Shared"].keys()], label="Shared", marker='o', color='#8B0000')
...
plt.grid()
plt.savefig("all_gpu_coords_2_speedup.png")
plt.close()
...
for i, method in enumerate(methods):
    for component, base_color in base_colors.items():
        alpha = 0.4 + 0.2 * i  # Adjust transparency to create distinct shades
...
# Save the plot
plt.tight_layout()
plt.savefig("all_gpu_coords_2_comparison.png")
plt.close()