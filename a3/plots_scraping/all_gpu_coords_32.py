...
with open(file_name, "r") as file:
    data_all_gpu = file.read()
    
all_data = data_naive + data_transpose + data_shared + data_all_gpu
...
t_seq_total = {method: t_alloc_seq[method] + t_run_seq[method] for method in methods if method in t_alloc_seq}
...
plt.plot(t_total["offload"].keys(), [t_seq_total["Naive"] / t_total["offload"][key] for key in t_total["offload"].keys()], label="Full-offload", marker='o', color='#FF8C00')
plt.legend()
...
plt.grid()
plt.savefig("all_gpu_coords_32_speedup.png")
plt.close()
...
# Save the plot
plt.tight_layout()
plt.savefig("all_gpu_coords_32_comparison.png")
plt.close()