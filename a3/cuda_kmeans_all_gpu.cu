...
void kmeans_gpu(double *objects,      /* in: [numObjs][numCoords] */
  ...
  cudaOccupancyMaxPotentialBlockSize(&minGridSize, &bestblockSize, find_nearest_cluster, numClusters * numCoords * sizeof(double), 0);
  printf("FullGPU kmeans, min_grid_size = %d, best_block_size = %d\n\n", minGridSize, bestblockSize);
  cudaOccupancyMaxPotentialBlockSize(&minGridSize, &bestblockSize, update_centroids, 0, 0);
  printf("FullGPU kmeans, min_grid_size = %d, best_block_size = %d\n\n", minGridSize, bestblockSize);
  */
