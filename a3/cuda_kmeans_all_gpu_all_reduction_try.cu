...
void find_nearest_cluster(int numCoords,
  ...
  // 1. `shmemClusters`: numClusters * numCoords doubles
  // 2. `shmem_newClusters`: numClusters * numCoords * blockDim.x doubles
  // 3. `shmem_newClustersSize`: numClusters * blockDim.x integers
  // 4. `delta_reduce_buff`: blockDim.x doubles

  // Offset computation
  double *shmemClusters = shmem_total;  // Starting at offset 0
  double *shmem_newClusters = shmemClusters + numClusters * numCoords; 
  int *shmem_newClustersSize = (int*)(shmem_newClusters + numClusters * numCoords * blockDim.x);
  double *delta_reduce_buff = (double*)(shmem_newClustersSize + numClusters * blockDim.x);
  ...
  if (tid < numObjs) {
    ...
    */
    ...
    //try reduction on the coords of the clusters as well
    int j;
    for (i = 0; i < numClusters; i++) {
      if (i != index) shmem_newClustersSize[threadIdx.x * numClusters + i] = 0;
      else shmem_newClustersSize[threadIdx.x * numClusters + i] = 1;
    }

    for (i = 0; i < numCoords; i++) {
      for (j = 0; j < numClusters; j++) {
        if (j != index) shmem_newClusters[threadIdx.x * numCoords * numClusters + i * numClusters + j] = 0.0;
        else shmem_newClusters[threadIdx.x * numCoords * numClusters + i * numClusters + j] = deviceobjects[i * numObjs + tid];
      }
    }
    __syncthreads();
    //after everyone has updated the newClusters coords and size do the reduction
    i = blockDim.x / 2;
    int k;
    while(i != 0) {
      if (threadIdx.x < i) {
        for (j = 0; j < numCoords; j++) {
          for (k = 0; k < numClusters; k++) {
            shmem_newClusters[threadIdx.x * numCoords * numClusters + j * numClusters + k] += shmem_newClusters[(threadIdx.x + i) * numCoords * numClusters + j * numClusters + k];
          }
        }
        for (j = 0; j < numClusters; j++) {
          shmem_newClustersSize[threadIdx.x * numClusters + j] += shmem_newClustersSize[(threadIdx.x + i) * numClusters + j];
        }
    ...
    if (threadIdx.x == 0) {
      for (i = 0; i < numCoords; i++) {
        for (j = 0; j < numClusters; j++) {
          //the result will be in the first thread resultsd of the shmem_newClusters
          atomicAdd(&devicenewClusters[i * numClusters + j], shmem_newClusters[i * numClusters + j]);
        }
      }
      for (i = 0; i < numClusters; i++) atomicAdd(&devicenewClusterSize[i], shmem_newClustersSize[i]);
    }
...
void kmeans_gpu(double *objects,      /* in: [numObjs][numCoords] */
  ...
  const unsigned int clusterBlockSharedDataSize = numClusters * numCoords * sizeof(double) //for share device Clusters
                                                  + numThreadsPerClusterBlock * numClusters * numCoords * sizeof(double) //for shared newClusters
                                                  + numThreadsPerClusterBlock * numClusters * sizeof(int) // for shared newClusterSize
