...
\
Το kernel find_nearest_cluster χρειάζεται να κληθεί με περισσότερες παραμέτρους και ίδιο μέγεθος shared memory, ενώ η κλήση της update_centroid δεν χρειάζεται καθόλου shared memory.
#my_sourcefile(
...
2) Το blocksize για τις πολλές συντεταγμένες (32) έχει παρόμοια επιρροή με τις υπόλοιπες εκδοχές και έχει βέλτιστη επίδοση για τα 2 μικρότερα block sizes. Όπως έχει προαναφερθεί, τα μικρά blocksizes έχουν την μέγιστη ευελιξία για το scheduling και είναι λογικό, αφού τα δεδομένα είναι πλήρως ανεξάρτητα, να έχουν καλύτερες επιδόσεις. Το blocksize για τις λίγες συντεταγμένες (2) δεν επηρεάζει εμφανώς την επίδοση με εξαίρεση το 48 που λόγω half warps και ότι δεν είναι bottleneck η μνήμη, δεν αξιοποιεί πλήρως τους πόρους της GPU.\
...
\
Το blocksize έχει διαφορετικό ρόλο μόνο στο configuration με τις λίγες συντεταγμένες (2), καθώς χρειάζεται το blocksize να είναι επαρκώς μεγάλο ώστε η λογαριθμική του πολυπλοκότητα να είναι καλύτερη από την γραμμική που έχει η απλή all-gpu. Σε μικρά blocksizes, η βελτίωση στην επίδοση που υπάρχει είναι μικρότερη καθώς η σταθερά αυτής της πολυπλοκότητας είναι αρκετά μεγάλη. 
\
== Παράρτημα 
\
Για την δημιουργία των γραφικών παραστάσεων χρημιοποίηθηκαν αρκετοί διαφορετικοί κώδικες σε Python. Ενδεικτικά για την σύγκριση όλων των εκδοχών εκτός της delta reduction, χρησιμοποιώθηκε ο παρακάτω κώδικας:
