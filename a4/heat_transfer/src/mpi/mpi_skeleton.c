...
int main(int argc, char ** argv) {
    ...
    //MPI_PROC_NULL if no neighbor exists
    ...
    // find useless rows and columns due to padding
    int useless_rows = global_padded[0] - global[0];
    int useless_columns = global_padded[1] - global[1];

    // the last row and column of processes will have to not take into account the useless rows and columns
    if (rank_grid[0] == grid[0] - 1) {
        i_max = local[0]  -useless_rows;
    } else {
        i_max = local[0] + 1;
    }

    if (rank_grid[1] == grid[1] - 1) {
        j_max = local[1] - useless_columns;
    } else {
        j_max = local[1] + 1;
    }
	...
	*/
    ...
    for (t=0;t<T;t++) {
        ...
        if (north != MPI_PROC_NULL) {
            MPI_Isend(&u_previous[1][0], local[1] + 2, MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &request[req_count++]);
        }
        if (south != MPI_PROC_NULL) {
            MPI_Isend(&u_previous[local[0]][0], local[1] + 2, MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &request[req_count++]);
        }
        if (west != MPI_PROC_NULL) {
            MPI_Isend(&u_previous[0][1], 1, column, west, 0, MPI_COMM_WORLD, &request[req_count++]);
        }
        if (east != MPI_PROC_NULL) {
            MPI_Isend(&u_previous[0][local[1]], 1, column, east, 0, MPI_COMM_WORLD, &request[req_count++]);
        }
        //receives
        if (north != MPI_PROC_NULL) {
            MPI_Irecv(&u_previous[0][0], local[1] + 2, MPI_DOUBLE, north, 0, MPI_COMM_WORLD, &request[req_count++]);
        }
        if (south != MPI_PROC_NULL) {
            MPI_Irecv(&u_previous[local[0]+1][1], local[1] + 2, MPI_DOUBLE, south, 0, MPI_COMM_WORLD, &request[req_count++]);
        }
        if (west != MPI_PROC_NULL) {
            MPI_Irecv(&u_previous[0][0], 1, column, west, 0, MPI_COMM_WORLD, &request[req_count++]);
        }
        if (east != MPI_PROC_NULL) {
            MPI_Irecv(&u_previous[0][local[1] + 1], 1, column, east, 0, MPI_COMM_WORLD, &request[req_count++]);
        }
	...
	//*************TODO*******************//
    MPI_Gatherv(&u_previous[1][1], 1, local_block, U[0], scattercounts, scatteroffset, global_block, 0, MPI_COMM_WORLD);
